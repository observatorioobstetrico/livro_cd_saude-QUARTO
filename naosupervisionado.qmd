...
K-médias é um método simples que consiste em anexar cada observação ao centróide mais perto. Dividimos o conjunto de dados para os respectivos centróides mais próximos, em seguida alteramos a posição dos $k$ centróides de forma a minimizar a soma dos quadrados dos erros em comparação com o posicionamento anterior, repetindo o processo até que a alteração na $SQE$ seja mínima. Ou seja, queremos:

$$
 \min_{c_i}\left( \sum_{i = 1}^k \sum_{x \in S_i}|| x - c_i|| ^2\right)
$$

Onde $C_i$ representa os centróides e cada $x$ uma observação.
